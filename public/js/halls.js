...
const today = new Date().toISOString().split("T")[0];
    ...
    async function openBookingModal(selectedHall = "") {
      hideFormError(bookingError);

      if (selectedHall) {
        localStorage.setItem("luminaSelectedHall", selectedHall);
      }
      ...
      const savedHall = selectedHall || localStorage.getItem("luminaSelectedHall") || "";
      if (savedHall) {
        setSelectValue(hallSelect, savedHall);
      }
    ...
    bookingButtons.forEach((button) => {
      button.addEventListener("click", function(event) {
        ...
        const selectedHall = button.dataset.hall || "";
        openBookingModal(selectedHall);
      });
    });
    ...
    bookingForm.addEventListener("submit", async function(event) {
      ...
      try {
        ...
        updatePhotographerRequirement();
        resetAvailableTimes();
        localStorage.removeItem("luminaSelectedHall");
