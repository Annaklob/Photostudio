...
const today = new Date().toISOString().split("T")[0];
    ...
    // ── НАВІГАЦІЯ: УВІЙТИ / МІЙ КАБІНЕТ ─────────────────────────────
    function updateNavAuth() {
      const navLink = document.getElementById("navAuthLink");
      if (!navLink) return;
      try {
        const saved = (sessionStorage.getItem("luminaUser") || localStorage.getItem("luminaUser"));
        if (!saved) return;
        const u = JSON.parse(saved);
        if (!u || !u.role) return;
        if (u.role === "client") {
          navLink.textContent = "Мій кабінет";
          navLink.href = "client-panel.html";
        } else if (u.role === "admin") {
          navLink.textContent = "Адмін-панель";
