...
const today = new Date().toISOString().split("T")[0];
    ...
    async function openBookingModal(selectedPhotographer = "") {
      hideFormError(bookingError);

      if (selectedPhotographer) {
        localStorage.setItem("luminaSelectedPhotographer", selectedPhotographer);
      }
      ...
      const savedPhotographer = selectedPhotographer || localStorage.getItem("luminaSelectedPhotographer") || "";
      if (savedPhotographer) {
        setSelectValue(photographerSelect, savedPhotographer);
      }
    ...
    bookingButtons.forEach((button) => {
      button.addEventListener("click", function(event) {
        ...
        const selectedPhotographer = button.dataset.photographer || "";
        openBookingModal(selectedPhotographer);
      });
    });
    ...
    bookingForm.addEventListener("submit", async function(event) {
      ...
      try {
        ...
        updatePhotographerRequirement();
        resetAvailableTimes();
        localStorage.removeItem("luminaSelectedPhotographer");
...
// ── ДИНАМІЧНІ ФОТОГРАФИ НА СТОРІНЦІ ──
function escapePhotographerHtml(value) {
  return String(value ?? "")
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function renderPublicPhotographers(photographers) {
  const list = document.querySelector(".photographers-list");
  if (!list) return;

  if (!Array.isArray(photographers) || photographers.length === 0) {
    list.innerHTML = `
      <div class="photographers-empty">
        Наразі активних фотографів немає.
      </div>
    `;
    return;
  }

  list.innerHTML = photographers.map((p) => {
    const fullName = escapePhotographerHtml(p.full_name || "Фотограф");
    const experience = escapePhotographerHtml(p.experience || "");
    const bio = escapePhotographerHtml(p.bio || "");
    const imageUrl = String(p.image_url || "").trim();

    const imageBlock = imageUrl
      ? `
        <img
          src="${escapePhotographerHtml(imageUrl)}"
          alt="${fullName}"
        >
      `
      : `
        <div class="photographer-image-placeholder">
          Фото буде додано
        </div>
      `;
    ...
    return `
      <article class="photographer-card">
        ...
        <div class="photographer-info">
          ...
          ${bio ? `<p>${bio}</p>` : `<p>Опис фотографа буде додано.</p>`}
          <a
            href="#"
            class="small-btn open-booking dynamic-photographer-booking"
            data-photographer="${fullName}"
          >
            Забронювати
          </a>
        </div>
...
async function loadPublicPhotographers() {
  const list = document.querySelector(".photographers-list");
  if (!list) return;
  ...
  try {
    ...
    if (!response.ok || !data.success) {
      throw new Error(data.message || "Не вдалося завантажити фотографів.");
    }

    renderPublicPhotographers(data.photographers || []);
  } catch (error) {
    list.innerHTML = `
      <div class="photographers-error">
        Не вдалося завантажити фотографів.
      </div>
    `;
  }
}
...
document.addEventListener("click", function(event) {
  const button = event.target.closest(".dynamic-photographer-booking");
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", loadPublicPhotographers);
} else {
  loadPublicPhotographers();
}
